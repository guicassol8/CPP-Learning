	...
	public:
		...
		int batalhaDeGatosRecursiva(){
			...
			if(gato1.front() <= gato1.size()-1 && gato2.front() <= gato2.size()-1){
				...
				int ganhador = batalhaDeGatosRecursiva();
				gato1 = aux1;
				gato2 = aux2;
				if (ganhador == 1){
			...
			return batalhaDeGatosRecursiva();
		}
		...
		bool inList (){
			string aux = toString();
			if (combinacoes.find(aux) != combinacoes.end()){
				return true;
			}
			return false;
		}
...
int main (){
	...
	getline(cin,player);
	NekoCafe.getBaralho(player);
	int ganhador = NekoCafe.batalhaDeGatosRecursiva();
